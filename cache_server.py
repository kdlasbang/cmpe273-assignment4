import sys
import socket
from pickle_hash import serialize, deserialize, hash_code_hex, serialize_GET
...
class MyDict(dict):
    ...
    def delete(self, key):
        if str(key.decode("utf-8")) in self:
                del self[str(key.decode("utf-8"))]
                return "success"
        else:
            return "failure--no this key"
